...
// DIY-ed by Ziyue
#define SPDK_TIME (__builtin_ia32_rdtsc())
...
namespace statistics{
    ...
    struct AvgStat{
      ...
      public:
        ...
        AvgStat(const std::string &s):
          sum_(0),
          size_(0),
          str(s){
        }
        void add(double value){
          __sync_fetch_and_add(&sum_, (uint64_t)(value * scale));
          __sync_fetch_and_add(&size_, 1);
        }
        double avg(){ return sum_ * 1.0 / scale / size_; }
        double sum(){ return sum_ * 1.0 / scale;}
        ...
        void print(std::string s){
          printf("%s: %lf / %ld = %lf\n", 
                s.c_str(), sum_*1.0/scale, size_, sum_*1.0/scale/size_);
        }
        void print(){
          printf("%s: %lf / %ld = %lf\n", 
                str.c_str(), sum_*1.0/scale, size_, sum_*1.0/scale/size_);
        }
    };*/
    ...
    struct AvgStat{
      ...
      public:
        ...
        void add(double value){
          int i = ssdlogging::random::Random::GetTLSInstance()->Next() % NUM;
    ...
    struct MySet{
      ...
      public:
        ...
        double print_tail() {
          ...
          printf("Avg: %.3lf (stat size: %ld, real size_: %ld), median: %.3lf, p9999: %.3lf, p999: %.3lf, p99: %.3lf, p90: %.3lf\n", \
        ...
        double print_from_last_end(uint64_t& step) {
          ...
          auto size = size_;
          auto sum = std::accumulate(data_ + cursor, data_ + size, 0.0);
          step = size - cursor;
          printf("Avg: %.3lf (stat size: %ld, size: %ld -> %ld)\n", \
                sum*1.0/step, step, cursor, size);
          fflush(stdout);
          cursor = size + 1;
          return sum*1.0/step;
        }
