...
void SPDKLoggingServer::AddLog(const char *data, int size, void *ptr) {
  ...
  // temp_time.add(SPDK_TIME_DURATION((static_cast<rocksdb::AsyncWriteCommittedTxn
  // *>(ptr))->start_time, SPDK_TIME));
  // auto start = SPDK_TIME;
  LoggingRequest *req = new LoggingRequest(data, size, ptr);
  // temp_time.add(SPDK_TIME_DURATION(
  // 	(static_cast<rocksdb::RequestScheduler::AsyncWriteRequest
  // *>(ptr))->start_time,
  //      start, spdk_tsc_rate_));

#ifdef DISRUPTOR_LOGGING_QUEUE
  submission_queue->WriteInBuf(req);
#else
...
inline uint64_t SPDKLoggingServer::MemCopy(LoggingRequest *leader,
  ...
  while (head != nullptr) {
    ...
    assert(offset + size + 4 <
           MAX_BUFFER_SIZE - 4 * (MAX_BUFFER_SIZE / PAGESIZE));
...
void SPDKLoggingServer::Server(int coreid, int serverid, int pipeline_num) {
  ...
  while (true) {
    ...
    if (serverid == 0 && last_print_time_.load() > 0 &&
        SPDK_TIME_DURATION(last_print_time_.load(), SPDK_TIME, spdk_tsc_rate_) >
            1000000) {
      double size = request_size.sum() * PAGESIZE / 1024.0 / 1024.0;
      printf("logging: %s %.2lf\n", GetDayTime().c_str(), size);
      // fflush(stdout);
    ...
    if (log_num != -1) {
#ifdef DISRUPTOR_LOGGING_QUEUE
      LoggingRequest *leader = submission_queue->ReadWithLock();
      if (leader != nullptr && leader->size_ < MAX_LOGGING_WRITE_SIZE) {
        uint64_t size = leader->size_;
        LoggingRequest *next = submission_queue->ReadWithLock();
        LoggingRequest *tail = leader;
        while (next != nullptr) {
          size += next->size_;
          tail->prev = next;
          tail = next;
          if (size > MAX_LOGGING_WRITE_SIZE) break;
          next = submission_queue->ReadWithLock();
        }
      }
#else
