...
namespace tstarling
{
  ...
  struct Learning_Result_Node
  {
    ...
    Learning_Result_Node(double ratio = 0, double metrics = 0,
                         double granularity = 0, bool status = false) : ratio_(ratio), metrics_(metrics), granularity_(granularity), status_(status) {}
  };

  struct Learning_Input_Node
  {
    double ratio_, granularity_;
    Learning_Input_Node(double ratio = 0, double granularity = 0) : ratio_(ratio), granularity_(granularity) {}
  };
...
#ifdef ADAPTIVE_CAP
    printf("Adaptive Cap is enabled\n");
#endif
  }
    ...
    while (!should_stop)
    {
      ...
      {                                    // cache is full
        if (last_miss_ratio <= miss_ratio) // Miss ratio is non-decreasing
          wait_count++;
        if (wait_count >= WAIT_STABLE_THRESHOLD)
        { // Waiting too long, stop filling process
          printf("- miss ratio = %.5lf -> %.5lf, with m_size = %lu (max = %lu)\n",
                 last_miss_ratio, miss_ratio, size, m_maxSize);
          break;
        }
  ...
  CONSTRUCT:
    ...
    while (!should_stop)
    {
      ...
      if (performance_depletion <= 0)
      {
        ...
        if (best_size > 0.25)
        {
          printf("Adaptive cap FC ratio from %.3lf to %.3lf\n", best_size, best_size - 0.25);
          best_size = best_size - 0.25;
          for (size_t i = 0; i < m_numShards; i++) // deconstruct all shards
          {
            m_shards[i]->deconstruct();
          }
          goto CONSTRUCT;
