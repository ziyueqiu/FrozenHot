...
#ifndef incl_LRU_CACHE_H
#define incl_LRU_CACHE_H
...
namespace Cache {
/**
 * LRUCache is a thread-safe hashtable with a limited size. When
...
class LRUCache : public BaseCacheAPI<TKey, TValue, THash> {
  /**
   * The LRU list node.
   *
...
typename LRUCache<TKey, TValue, THash>::ListNode* const
    LRUCache<TKey, TValue, THash>::OutOfListMarker = (ListNode*)-1;
...
bool LRUCache<TKey, TValue, THash>::find(TValue& ac,
  ...
  if (lock) {
    ...
    //LRUCache::promotion_counter++; // for test
  }
  if(LRUCache::sample_generator())
    LRUCache::hit_num++;
  return true;
...
void LRUCache<TKey, TValue, THash>::clear() {
  ...
  //printf("\npromotion counter: %d\n", LRUCache::promotion_counter.load());
}

template <class TKey, class TValue, class THash>
void LRUCache<TKey, TValue, THash>::snapshotKeys(
...
template <class TKey, class TValue, class THash>
inline void LRUCache<TKey, TValue, THash>::delink(ListNode* node) {
...
template <class TKey, class TValue, class THash>
inline void LRUCache<TKey, TValue, THash>::pushFront(ListNode* node) {
...
template <class TKey, class TValue, class THash>
void LRUCache<TKey, TValue, THash>::evict() {
...
// template <class TKey, class TValue, class THash>
// void LRUCache<TKey, TValue, THash>::evict_key() {
//   evict();
//   m_size--;
// }

template <class TKey, class TValue, class THash>
void LRUCache<TKey, TValue, THash>::delete_key(const TKey& key) {
