...
#ifndef incl_FIFO_CACHE_H
#define incl_FIFO_CACHE_H
...
namespace Cache {
/**
 * FIFOCache is a thread-safe hashtable with a limited size. When
...
class FIFOCache : public BaseCacheAPI<TKey, TValue, THash> {
  /**
   * The LRU list node.
   *
 ...
 public:
   ...
   */
  explicit FIFOCache(size_t maxSize);

  FIFOCache(const FIFOCache& other) = delete;
  FIFOCache& operator=(const FIFOCache&) = delete;

  virtual ~FIFOCache() { clear(); }
...
typename FIFOCache<TKey, TValue, THash>::ListNode* const
    FIFOCache<TKey, TValue, THash>::OutOfListMarker = (ListNode*)-1;

template <class TKey, class TValue, class THash>
FIFOCache<TKey, TValue, THash>::FIFOCache(size_t maxSize)
    : m_maxSize(maxSize),
      m_size(0),
...
bool FIFOCache<TKey, TValue, THash>::find(TValue& ac,
  ...
  if (lock) {
    ...
    //FIFOCache::promotion_counter++; // for test
  }*/
  if(FIFOCache::sample_generator())
    FIFOCache::hit_num++;
  return true;
...
template <class TKey, class TValue, class THash>
bool FIFOCache<TKey, TValue, THash>::insert(const TKey& key,
                                                     const TValue& value) {
  // __sync_fetch_and_add(&insert_num, 1);
  // Insert into the CHM
...
template <class TKey, class TValue, class THash>
void FIFOCache<TKey, TValue, THash>::clear() {
  m_map.clear();
  ...
  //printf("\npromotion counter: %d\n", FIFOCache::promotion_counter.load());
}

template <class TKey, class TValue, class THash>
void FIFOCache<TKey, TValue, THash>::snapshotKeys(
...
template <class TKey, class TValue, class THash>
inline void FIFOCache<TKey, TValue, THash>::delink(ListNode* node) {
...
template <class TKey, class TValue, class THash>
inline void FIFOCache<TKey, TValue, THash>::pushFront(ListNode* node) {
...
template <class TKey, class TValue, class THash>
void FIFOCache<TKey, TValue, THash>::evict() {
  std::unique_lock<ListMutex> lock(m_listMutex);
...
// template <class TKey, class TValue, class THash>
// void FIFOCache<TKey, TValue, THash>::evict_key() {
//   evict();
//   m_size--;
// }

template <class TKey, class TValue, class THash>
void FIFOCache<TKey, TValue, THash>::delete_key(const TKey& key) {
  HashMapConstAccessor hashAccessor;
  if (!m_map.find(hashAccessor, key)) {
    return;
  }
