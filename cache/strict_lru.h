...
#ifndef incl_Strict_LRU_CACHE_H
#define incl_Strict_LRU_CACHE_H
...
namespace Cache {
/**
 * StrictLRUCache is a thread-safe hashtable with a limited size. When
...
class StrictLRUCache : public BaseCacheAPI<TKey, TValue, THash> {
  /**
   * The LRU list node.
   *
 ...
 public:
   ...
   */
  explicit StrictLRUCache(size_t maxSize);

  StrictLRUCache(const StrictLRUCache& other) = delete;
  StrictLRUCache& operator=(const StrictLRUCache&) = delete;

  virtual ~StrictLRUCache() { clear(); }
 ...
 private:
  ...
  void fast_pushFront(ListNode* node);
...
typename StrictLRUCache<TKey, TValue, THash>::ListNode* const
    StrictLRUCache<TKey, TValue, THash>::OutOfListMarker = (ListNode*)-1;

template <class TKey, class TValue, class THash>
StrictLRUCache<TKey, TValue, THash>::StrictLRUCache(size_t maxSize)
    : m_maxSize(maxSize),
      m_size(0),
            ...
            4)  // it will automatically grow
{
  ...
  StrictLRUCache::reset_stat();
}

template <class TKey, class TValue, class THash>
bool StrictLRUCache<TKey, TValue, THash>::find(TValue& ac,
                                                   const TKey& key) {
  HashMapConstAccessor hashAccessor;
  //HashMapAccessor& hashAccessor = ac.m_hashAccessor;
  if (!m_map.find(hashAccessor, key)) {
    if(StrictLRUCache::sample_generator())
      StrictLRUCache::miss_num++;
    return false;
  }
...
template <class TKey, class TValue, class THash>
void StrictLRUCache<TKey, TValue, THash>::clear() {
  m_map.clear();
...
template <class TKey, class TValue, class THash>
void StrictLRUCache<TKey, TValue, THash>::snapshotKeys(
...
template <class TKey, class TValue, class THash>
inline void StrictLRUCache<TKey, TValue, THash>::delink(ListNode* node) {
...
template <class TKey, class TValue, class THash>
inline void StrictLRUCache<TKey, TValue, THash>::pushFront(ListNode* node) {
...
template <class TKey, class TValue, class THash>
void StrictLRUCache<TKey, TValue, THash>::evict() {
  std::unique_lock<ListMutex> lock(m_listMutex);
...
// template <class TKey, class TValue, class THash>
// void StrictLRUCache<TKey, TValue, THash>::evict_key() {
//   evict();
//   m_size--;
// }

template <class TKey, class TValue, class THash>
void StrictLRUCache<TKey, TValue, THash>::delete_key(const TKey& key) {
  HashMapConstAccessor hashAccessor;
  if (!m_map.find(hashAccessor, key)) {
    return;
  }
